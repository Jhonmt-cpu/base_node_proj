...
describe("Get User Complete Me Controller", () => {
  beforeAll(async () => {
  ...
  it("should be able to get a user complete with authentication and create cache", async () => {
    ...
    const cacheKey = `${cachePrefixes.getUserComplete}:${user.user_id}`;

    const cacheValueBefore = await cacheProvider.cacheGet(cacheKey);

    const response = await request(app)
      .get(`/account/user/me/complete`)
      .set({
  ...
  it("should be able to get an user complete from cache", async () => {
    const { user_email, user_password } = testConfig.user_test;
  ...
  it("should not be able to get a non existing user complete", async () => {
  ...
  it("should not be able to get a user complete without authentication", async () => {
    const response = await request(app).get(`/account/user/me/complete`);
